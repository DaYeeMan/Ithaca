...
## September 8 content and protection follow-up

Production at https://capitalcanvas.vercel.app passed direct-route rendering and the same-origin HTTP pricing smoke (10.4505835722). The previously reported SPA 404 is resolved on that deployed build.

Local finalization removes review labels, footer email and repeated investment sentence, and the About location sentence. Contact remains in About and legal pages. KaTeX is used directly; package license notices, Plotly's supplied bundle notices, and font-embedded copyright/OFL notices are generated by `python scripts/generate_notices.py`. No fixed provider log deletion promise is made. Source audit confirms application logs contain path/method/status/timing/request IDs, not calculation request bodies. The clean browser check found no storage/cookies; this observation is not a guarantee about every provider security flow.

Firewall inspection returned no existing custom rules. A single log-only rule is staged: `Ithaca solve rate` (`rule_ithaca_solve_rate_tXl5Aq`), POST /v1/solve, 60 requests per IP per 60 seconds, fixed window, action on exceed = log. The staged diff contains only that addition. It does not enforce or become live until published. Owner command: `npx --yes vercel@59.11.7 firewall publish --yes`. Review traffic before changing the exceed action to rate_limit and publishing enforcement. Do not describe this observation stage as active throttling.
